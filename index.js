...
const mongoURL = process.env.MONGO_URL || 'mongodb://localhost:27017/appdist'
...
async function start() {
  ...
  app.get(
    ...
    console.log(req.params.id)
     await deleteNote(db, {_id: parseId(req.params.id)})
      res.redirect("/");
    }
  );
const parseId=(id)=>{
...
async function retrieveNotes(db) {
  ...
  return sortedNotes.map(it => ({ ...it, description: marked(it.description), id: it._id}))
}
